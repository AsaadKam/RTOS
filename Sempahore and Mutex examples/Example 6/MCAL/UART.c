...
static volatile uinteg8_t  sgu8_UART_REC_BYTE=0;
static volatile PntrToFunc_t gPntrToFunc_UART_TX_ISR=NullPointer,gPntrToFunc_UART_RX_ISR=NullPointer;
static uinteg8_t sgu8_UART_Execution_Mode=0;
static uinteg8_t sgu8_UART_Receiver_enable=0;
static uinteg8_t sgu8_UART_Transimter_enable=0;


/*- GLOBAL EXTERN VARIABLES -------------------------------*/

 #if UART_ISR_WITH_FLAG==FLAG
volatile uinteg8_t gu8_Uart_rec_char_flag=0U;
volatile uinteg8_t gu8_Uart_send_char_flag=0U;
#elif UART_ISR_WITH_FLAG==COUNTER
volatile uinteg8_t gu8_Uart_rec_char_counter=0U;
volatile uinteg8_t gu8_Uart_send_char_counter=0U;
#endif
...
UART_Error_t UART_Init(const UART_Confg_Stuct_t* pstr_Config_UART)
{
    ...
    if(pstr_Config_UART!=NullPointer)
	{
		...
		/**Set baud rate**/
		UART_BAUD_RATE_REG_HIGH = (uinteg8_t)((pstr_Config_UART->BaudRate)>>8);
		UART_BAUD_RATE_REG_LOW =  (uinteg8_t) (pstr_Config_UART->BaudRate);
		...
		{
			sgu8_UART_Execution_Mode=pstr_Config_UART->InterruptMode;			
		}
	}
	else 
	{
 ...
 * Output    : Error Checking
 *_______________________________________________________________________________________________________________________________*/
UART_Error_t UART_SendByte(uinteg8_t u8_Byte_UART)
{
 ...
 *              1-(uinteg8_t* pchar_index)It takes pointer to char 
 * Output    : Error Checking
 *_______________________________________________________________________________________________________________________________*/
UART_Error_t UART_SendString(uinteg8_t* pchar_index)
{
  if(sgu8_UART_Execution_Mode==UART_Polling_mode_enable)
  {
	while(*(pchar_index))
